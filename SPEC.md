...
V19hm: ∀ `astral-sh/setup-uv` workflow refs ! use the currently pinned published tag; matching regression test updates with Dependabot bumps
...
B5qa|2026-08-18|Dependabot updated workflow refs from `astral-sh/setup-uv@v9.0.0` to `@v10.0.1` but the regression test retained the old pinned tag|update test expectation with the workflow pin; V19hm
